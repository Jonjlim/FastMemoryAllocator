...
span_t *cmalloc_initialize_span(void *ptr, size_t size, int size_class_index) {
    ...
    cmalloc_map_range(page_map, new, get_page_index(new), get_page_index(new) + ((new->span_size + PAGE_SIZE - 1) >> PAGE_SHIFT));
...
void cmalloc_uninitialize_span(span_t *span) {
    cmalloc_unmap_range(page_map, get_page_index(span), get_page_index(span) + ((span->span_size + PAGE_SIZE - 1) >> PAGE_SHIFT));
}

span_t *cmalloc_get_span(void *ptr) {
    return cmalloc_get(page_map, get_page_index(ptr));
}
